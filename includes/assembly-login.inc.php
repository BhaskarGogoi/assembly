<?php
	...
	if(isset($_POST['submit'])) {
		...
		if ($_POST['captcha_word'] != $code) {
			header ("Location:../index?error=captcha");
			exit();
		}
		// ------End Captcha------
		...
		//-----Check if form datas are not filled-----
			...
			//Create a template
			$sql = "SELECT * FROM assembly_login WHERE username = ?  AND password = ?; ";
			...
			if (!mysqli_stmt_prepare($stmt, $sql)) {
				...
				if (!$row = $result->fetch_assoc()) {
					header ("Location:../index?error=username-or-password");
					exit();
				} else {

					$sql = "INSERT INTO assembly_login_details (username, ip, operating_system, browser, date, time)
					...
					if ($result) {
						...
						header ("Location: ../index?error=getting-login-details");
						exit();
					}
				}
			}
